...
class CoronnaCERTAINDataset():
    ...
    def __init__(self,
        ...
        Args:
            ...
            dataset: Dataset used for modeling
    ...
    def create_dataframe(self, data_file, time_points, dataset):
        ...
        df1.loc[:,'DAS28_CRP'] = df1.apply(lambda row: self.calculate_DAS28_CRP(row), axis = 1)
        df2.loc[:,'DAS28_CRP'] = df2.apply(lambda row: self.calculate_DAS28_CRP(row), axis = 1)
        ...
        if self.save_csv:
            file_loc = f'../Dataset/Coronna_Data_CERTAIN_{approach}_{time_1}M_{time_2}M_{subset}_{dataset}.csv'
            sys.stdout.write("save file to:",file_loc)
            sys.stdout.flush()
